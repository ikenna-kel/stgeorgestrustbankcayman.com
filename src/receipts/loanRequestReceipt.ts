...
export type LoanRequestReceiptData = {
  requestId: string;
  createdAt: string;
  status?: string;
...
export async function buildLoanRequestPdf(data: LoanRequestReceiptData) {
  const doc = new jsPDF();
  ...
  doc.setFontSize(16);
  doc.text("LOAN REQUEST RECEIPT", pageWidth - margin, 20, { align: "right" });
  ...
  const rows: Array<[string, string]> = [
    ...
    ["Term", `${data.termMonths} month(s)`],
  ];

  if (data.dueDate) rows.push(["Due Date", new Date(data.dueDate).toLocaleDateString()]);
...
export async function downloadLoanRequestPdf(data: LoanRequestReceiptData) {
  const doc = await buildLoanRequestPdf(data);
  doc.save(`loan-request-${data.requestId}.pdf`);
}
